[2026-03-17T17:53:08Z] Invoking agent: pr-assembler (anthropic/claude-opus-4.6)
## Summary
...
The Forge pipeline relies on a well-defined contract between agents. Each artifact serves as both an output of one agent and an input to downstream agents. Establishing these files ensures:

1. **Reproducibility** — Every pipeline run produces a consistent set of artifacts that can be audited.
2. **Decoupled agents** — Agents communicate through structured JSON/Markdown files rather than direct coupling.
3. **Observability** — The pipeline log and individual reports make it straightforward to diagnose failures or regressions.
4. **Automation** — The PR description is assembled automatically from upstream agent outputs, reducing manual overhead.
...
- All JSON artifacts follow a consistent schema contract to ensure downstream agents can parse them reliably.
- No production source code was modified — this change is scoped entirely to pipeline infrastructure and artifacts.
